...
def across_paths(stn_paths, execution, threads, sim_count, sim_options,
    ...
    for i, pair in enumerate(stn_pairs):
        ...
        if ordering_pairs is not None:
            ...
            results_dict = run(pair, execution, sim_count, threads,
...
def simulate(starting_stn, execution_strat,
    ...
    # one run
    simulator = Simulator(seeds[0])

    # Sucessful or unsuccesful simulation?
    ans = simulator.simulate(starting_stn, execution_strat, sim_options)
...
def run(pair, execution, sim_count, threads, random_seed, sim_options):

    path, stn = pair

    start_time = time.time()
    response_dict = simulate(stn, execution, sim_count,
                                         threads=threads,
                                         random_seed=random_seed,
                                         sim_options=sim_options)
...
def _multisim_thread_helper(tup):
    ...
    """
    pr.verbose("Multisim thread helper: {}".format(tup))
    simulator = tup[0]
    if tup[2] == "da":
